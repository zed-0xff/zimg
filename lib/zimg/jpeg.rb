  ...
  def _read io
    ...
    while !io.eof?
      ...
      when 0xc0..0xc2
        ...
        @width, @height, @bpp = chunk.width, chunk.height, chunk.bpp
        @sof = chunk
      ...
      @chunks.each do |c|
        ...
        when SOS
          ...
          components = sos.components.map do |idx, table_spec|
            c = frame.components.find{ |c| c.id == idx }
            c.huffman_table_dc = huffman_tables_dc[table_spec >> 4]
            c.huffman_table_ac = huffman_tables_ac[table_spec & 15]
            c
          end
