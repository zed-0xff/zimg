...
module ZIMG
  module PNG
    class Scanline
      ...
      VALID_FILTERS = (FILTER_NONE..FILTER_PAETH).freeze
      ...
      def initialize(image, idx, params = {})
        ...
        if @image.new?
          ...
          if (@filter = image.imagedata[@offset])
            ...
            warn "[!] #{self.class}: ##{@idx}: no data at pos 0, scanline dropped".red
          end
        end
        @errors = Set.new
      end
      ...
      def inspect
        if image.interlaced?
          format("#<Scanline idx=%-2d offset=%-3d width=%-2d size=%-2d bpp=%d filter=%d>", idx, offset, width, size,
            bpp, filter)
        else
          format("#<Scanline idx=%-2d offset=%-3d size=%-2d bpp=%d filter=%d>", idx, offset, size, bpp, filter)
        end
      end
      ...
      def []=(x, color)
        ...
        when COLOR_RGB # ALLOWED_DEPTHS: 8, 16
          case @bpp
          when 24 then decoded_bytes[x * 3, 3] = color.to_depth(8).to_a.pack("C3")
          when 48 then decoded_bytes[x * 6, 6] = color.to_depth(16).to_a.pack("n3")
        ...
        when COLOR_GRAY_ALPHA             # ALLOWED_DEPTHS: 8, 16
          case @bpp
          when 16 then decoded_bytes[x * 2, 2] = color.to_depth(8).to_gray_alpha.pack("C2")
          when 32 then decoded_bytes[x * 4, 4] = color.to_depth(16).to_gray_alpha.pack("n2")
          else raise "unexpected bpp #{@bpp}"
          end

        when COLOR_RGBA                   # ALLOWED_DEPTHS: 8, 16
          case @bpp
          when 32 then decoded_bytes[x * 4, 4] = color.to_depth(8).to_a.pack("C4")
          when 64 then decoded_bytes[x * 8, 8] = color.to_depth(16).to_a.pack("n4")
          else raise "unexpected bpp #{@bpp}"
          end
      ...
      def get_raw(x)
        ...
        raw =
          if @bytes_per_pixel
            ...
            decoded_bytes[x * @bytes_per_pixel, @bytes_per_pixel]
          else
      ...
      def [](x)
        ...
        when COLOR_INDEXED # ALLOWED_DEPTHS: 1, 2, 4, 8
          ...
          unless color
            if !@errors.include?(x) && @image.verbose >= -1
              ...
              @errors << x
              msg = if (32..127).include?(idx)
                      format('[!] %s: color #%-3d ("%c") at x=%d y=%d is out of palette!'.red, self.class, idx, idx, x,
                        @idx)
                    else
                      format("[!] %s: color #%-3d at x=%d y=%d is out of palette!".red, self.class, idx, x, @idx)
                    end
              warn msg
            end
          ...
          #   ...
          #
          if image.trns && ((color.alpha = image.trns.data[idx]))
            # if it's not NULL - convert it from char to int,
            # otherwise it means fully opaque color, as well as NULL alpha in Color
            color = color.dup
        ...
        when COLOR_GRAY_ALPHA             # ALLOWED_DEPTHS: 8, 16
          ...
          when 16                         #  8-bit grayscale +  8-bit alpha
            Color.from_grayscale(*raw.unpack("C2"))
          when 32                         # 16-bit grayscale + 16-bit alpha
            Color.from_grayscale(*raw.unpack("n2"), depth: 16)
        ...
        when COLOR_RGBA                   # ALLOWED_DEPTHS: 8, 16
          ...
          when 64                         # RGBA 16-bit/sample
            Color.new(*raw.unpack("n4"), depth: 16)
        ...
        @decoded_bytes ||=
          begin
            ...
            when FILTER_PAETH   # 4
              ...
              warn "[!] #{self.class}: ##{@idx}: invalid filter #{@filter}, assuming FILTER_NONE".red
              s = raw
            end
      ...
      def raw_data=(data)
        raise StandardError, "raw data size must be #{size}, got #{data.size}" unless data.size == size
      ...
      def crop!(x, w)
        ...
        if @bytes_per_pixel
          ...
          decoded_bytes[0, x * @bytes_per_pixel] = ""
          decoded_bytes[w * @bytes_per_pixel..-1] = ""
        else
